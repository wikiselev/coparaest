...
#
# This script reads indexes of the best objective values defined before from
# best_obj_values.txt file and plot time series of all system components
# corresponding to the indexes of objective values
#
...
file_ind <- read.table("best_obj_values.txt")$V1
obj_vals <- read.table("best_obj_values.txt")$V2
...
for (i in file_ind) {
	...
	wt_est <- read.table(paste("../output/wt_est/wt_est", i, ".txt", sep=""), 
	...
	wt_est$obj_vals <- rep(paste(obj_vals[j], " - ", i, sep=""), length(wt_est[ ,1]))
	rownames(wt_est) <- NULL

	# import the data table with estimated pten time courses
	pten_est <- read.table(paste("../output/pten_est/pten_est", i, ".txt", sep=""), 
		header = TRUE, sep = "\t", stringsAsFactors = FALSE)
	...
	pten_est$obj_vals <- rep(paste(obj_vals[j], " - ", i, sep=""), length(pten_est[ ,1]))
	rownames(pten_est) <- NULL

	# import the data table with experimental wt time courses
	wt_exp <- read.table("experiment/simple/wt_lit.txt", header = TRUE, sep = "\t", 
	...
	wt_exp$obj_vals <- rep(paste(obj_vals[j], " - ", i, sep=""), length(wt_exp[ ,1]))
	rownames(wt_exp) <- NULL

	# import the data table with experimental pten time courses
	pten_exp <- read.table("experiment/simple/pten_lit.txt", header = TRUE, sep = "\t", 
		stringsAsFactors = FALSE)
	...
	pten_exp$obj_vals <- rep(paste(obj_vals[j], " - ", i, sep=""), length(pten_exp[ ,1]))
	rownames(pten_exp) <- NULL
	...
	p <- ggplot(total.data.pip, aes(x=time, y=value)) + facet_wrap(~ variable, 
		...
		ggtitle(paste(obj_vals[j], " - ", i, sep=""))
	pdf(file = paste("results/pips", j, ".pdf", sep=""), width = 6, height = 4)
	print(p)
	dev.off()
	...
	p1 <- ggplot(total.data.rest, aes(x=time, y=value)) + facet_wrap(~ variable, 
		...
		ggtitle(paste(obj_vals[j], " - ", i, sep=""))
	pdf(file = paste("results/rest", j, ".pdf", sep=""), width = 10, height = 3)
	print(p1)
	dev.off()

	j <- j + 1 # use the next objective value in the file
}
...
p <- ggplot(all.est.pip, aes(x=time, y=value)) + facet_grid(variable ~ obj_vals, 
	scales = "free_y") + geom_line(aes(color = cond, linetype = run))
pdf(file = "results/pips_all.pdf", width = 6, height = 3)
print(p)
dev.off()
